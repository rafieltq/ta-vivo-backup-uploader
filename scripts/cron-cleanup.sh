...
# Crontab-friendly wrapper for cleanup
# Usage: ./scripts/cron-cleanup.sh
...
if make cleanup-daily >> "$LOG_FILE" 2>&1; then
    log "Cleanup completed successfully"
    # Keep only last 30 days of cleanup logs
    find logs -name "cleanup-*.log" -mtime +30 -delete 2>/dev/null || true
    exit 0
else
